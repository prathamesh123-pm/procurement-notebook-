...
export default function ReportsPage() {
  ...
  return (
    <div className="max-w-full mx-auto w-full pb-10 animate-in fade-in duration-500 print:p-0 overflow-x-hidden">
      ...
      <div className="flex flex-col gap-2 no-print px-2">
        <div className="flex items-center justify-between">
          <h2 className="text-base font-black text-foreground flex items-center gap-2">
            <Archive className="h-4 w-4 text-primary" /> अहवाल (Reports)
          </h2>
          <Badge className="bg-primary/10 text-primary font-black text-[10px] h-5 px-2 border-none">
          ...
          <div className="flex gap-1.5 pb-2">
            {['All', 'Route Visit', 'Field Visit', 'Daily Office Work', 'Daily Task', 'Breakdown'].map((type) => (
              <Button 
                ...
                className={`font-black rounded-lg px-3 text-[10px] h-7 flex-shrink-0 ${activeFilter === type ? 'shadow-sm' : 'bg-white'}`}
              >
        ...
        <div className="flex items-center gap-2 bg-white p-2 rounded-lg border shadow-sm w-full">
          ...
          <Input 
            type="date" 
            className="h-8 w-full text-[11px] font-black bg-muted/20 border-none rounded-md focus-visible:ring-0" 
            value={filterDate}
      ...
      <div className="grid grid-cols-1 gap-2 mt-3 no-print px-2">
        {filteredReports.length > 0 ? (
          filteredReports.map((report) => (
            <Card key={report.id} className="border shadow-none overflow-hidden bg-white rounded-xl border-l-4 border-l-primary">
              <CardContent className="p-3">
                <div className="flex flex-col gap-2">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                      <div className={`p-1.5 rounded-lg ${getTypeColor(report.type)}`}>
                        {getIcon(report.type)}
                      </div>
                      <div className="min-w-0">
                        <h4 className="font-black text-[12px] text-foreground truncate">{report.type}</h4>
                    ...
                    <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive rounded-full hover:bg-destructive/5 shrink-0" onClick={() => handleDelete(report.id)}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                  
                  <div className="text-[11px] text-muted-foreground bg-muted/10 p-2 rounded-lg italic leading-tight border border-dashed whitespace-normal break-words">
                    {report.summary}
                  </div>

                  <div className="flex justify-end gap-2 pt-1">
                    <Button variant="outline" size="sm" onClick={() => handleViewReport(report)} className="font-black text-[10px] h-8 rounded-lg px-4 border-primary/20 text-primary hover:bg-primary/5">
                      <Eye className="h-3.5 w-3.5 mr-1.5" /> पहा
                    </Button>
                    <Button size="sm" onClick={() => { setSelectedReport(report); setTimeout(handleDownloadPDF, 100); }} className="font-black text-[10px] h-8 rounded-lg bg-primary text-white shadow-sm px-4">
                      <Download className="h-3.5 w-3.5 mr-1.5" /> प्रिंट
                    </Button>
                  </div>
                </div>
              </CardContent>
          ...
          <div className="text-center py-16 bg-white rounded-2xl border-2 border-dashed border-muted-foreground/10 flex flex-col items-center gap-3">
             <Archive className="h-8 w-8 text-muted-foreground/20" />
             <h3 className="text-[11px] font-black text-muted-foreground uppercase tracking-widest">एकही अहवाल सापडला नाही</h3>
          </div>
        )}
      </div>
      ...
      <Dialog open={isViewOpen} onOpenChange={setIsViewOpen}>
        <DialogContent className="max-w-2xl h-[90vh] flex flex-col p-0 bg-white overflow-hidden rounded-none sm:rounded-xl border-none">
          <DialogHeader className="p-3 border-b no-print bg-primary/5 flex flex-row items-center justify-between shrink-0">
            <DialogTitle className="text-xs font-black flex items-center gap-2 px-2">
              <FileText className="h-4 w-4 text-primary" /> अहवाल पहा
            </DialogTitle>
            <div className="flex gap-2 pr-8">
              <Button size="sm" className="gap-1.5 font-black rounded-lg bg-primary h-8 text-[10px] px-3" onClick={handleDownloadPDF}><Printer className="h-3.5 w-3.5" /> प्रिंट</Button>
            ...
            {selectedReport && (
              <div className="p-4 sm:p-6 space-y-4 bg-white" id="printable-report-content">
                <div className="flex flex-col items-center border-b-2 border-black pb-3 text-center space-y-2">
                  ...
                  <div className="grid grid-cols-3 w-full mt-2 text-[10px] font-black uppercase border-t border-gray-100 pt-3">
                    <div className="flex flex-col text-left"><span className="text-gray-400 text-[8px]">नाव:</span> {profileName || selectedReport.fullData?.name || 'N/A'}</div>
                    <div className="flex flex-col text-center"><span className="text-gray-400 text-[8px]">तारीख:</span> {selectedReport.date}</div>
                    <div className="flex flex-col text-right"><span className="text-gray-400 text-[8px]">शिफ्ट:</span> {selectedReport.fullData?.shift || 'N/A'}</div>
                  </div>
                </div>
                ...
                {selectedReport.type === 'Route Visit' && (
                  <div className="space-y-3">
                    <div className="grid grid-cols-4 gap-2 p-2 border rounded-xl bg-gray-50/50">
                      <div><Label className="text-[8px] font-black uppercase text-gray-500">वाहन</Label><p className="text-[10px] font-black">{selectedReport.fullData?.vehicleNumber || '-'}</p></div>
                      <div><Label className="text-[8px] font-black uppercase text-gray-500">KM</Label><p className="text-[10px] font-black text-blue-700">{selectedReport.fullData?.totalKm || '0'}</p></div>
                      <div><Label className="text-[8px] font-black uppercase text-gray-500">वेळ</Label><p className="text-[10px] font-black">{selectedReport.fullData?.routeOutTime || '-'}/{selectedReport.fullData?.routeInTime || '-'}</p></div>
                      <div className="text-right"><Label className="text-[8px] font-black uppercase text-gray-500">तूट</Label><p className="text-[10px] font-black text-red-600">{selectedReport.fullData?.shortageLiters || '0'} L</p></div>
                    </div>
                          ...
                          {selectedReport.fullData?.routeVisitLogs?.map((log: any, idx: number) => (
                            <tr key={idx} className="border-b border-gray-100 last:border-0 odd:bg-white even:bg-gray-50/30">
                              ...
                              <td className="p-2">
                                ...
                                <p className="text-[9px] text-gray-500">{log.supplierName}</p>
                              </td>
                ...
                {selectedReport.type === 'Breakdown' && (
                  <div className="space-y-3">
                    <div className="p-3 border-2 border-red-600 rounded-xl bg-red-50/30 space-y-2">
                      <div className="grid grid-cols-2 gap-2 border-b border-red-100 pb-2">
                        <div><Label className="text-[8px] font-black text-gray-400 uppercase">रूट</Label><p className="text-[11px] font-black">{selectedReport.fullData?.routeName}</p></div>
                        <div className="text-right"><Label className="text-[8px] font-black text-gray-400 uppercase">गाडी</Label><p className="text-[11px] font-black">{selectedReport.fullData?.vehicleNumber}</p></div>
                      </div>
                      <div className="p-2 bg-white border rounded-lg">
                        <Label className="text-[8px] font-black text-red-600 uppercase">कारण</Label>
                      ...
                      <table className="w-full text-[10px] border-collapse">
                        ...
                        <tbody>
                          ...
                          <tr className="bg-gray-50">
                            <td className="p-2 font-black text-[10px]">एकूण</td>
                            <td colSpan={3} className="p-2 text-right font-black text-[11px] text-red-700">₹{selectedReport.fullData?.totalLossAmount}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                ...
                {selectedReport.type === 'Daily Task' && (
                  <div className="space-y-2">
                    <div className="p-3 border-2 border-black rounded-xl bg-gray-50 space-y-2">
                      <div className="flex justify-between items-start border-b border-gray-200 pb-2">
                        <div className="min-w-0">
                          <Label className="text-[8px] font-black text-gray-400 uppercase">गवळी</Label>
                          <p className="text-[11px] font-black truncate">{selectedReport.fullData?.supplierName || "N/A"}</p>
                        </div>
                        <div className="text-right">
                          <Label className="text-[8px] font-black text-gray-400 uppercase">टास्क</Label>
                          <p className="text-[11px] font-black">{selectedReport.fullData?.title}</p>
                        </div>
                      </div>
                      <div className="p-2 bg-blue-50 border-l-4 border-primary rounded-r-lg">
                        <Label className="text-[8px] font-black text-primary uppercase">कार्यवाही / शेरा</Label>
                        <p className="text-[11px] font-black text-gray-800 leading-tight whitespace-pre-wrap">{selectedReport.fullData?.remark || "कोणतीही नोंद नाही."}</p>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {(selectedReport.type === 'Field Visit' || selectedReport.type === 'Daily Office Work') && (
                  <div className="p-3 border border-black rounded-xl bg-gray-50/30">
                    <Label className="text-[9px] font-black uppercase mb-2 block text-gray-400 tracking-widest">Observations / Work Detail</Label>
                    <div className="bg-white p-3 rounded-lg border border-gray-200 min-h-[120px] shadow-sm">
                  ...
                  <div className="flex justify-between items-end gap-12 px-4">
                    <div className="text-center flex-1 space-y-2">
                      <div className="border-b border-black pb-1">
                        <span className="text-[11px] font-black">{profileName || '-----------------'}</span>
                      </div>
                      <span className="text-[8px] font-black uppercase text-gray-400 tracking-tighter">प्रतिनिधी स्वाक्षरी</span>
                    </div>
                    <div className="text-center flex-1 space-y-2">
                      <div className="border-b border-black pb-1">
                        <span className="text-[11px] font-black text-transparent">SIGNATURE</span>
                      </div>
                      <span className="text-[8px] font-black uppercase text-gray-400 tracking-tighter">सुपरवायझर स्वाक्षरी</span>
                    </div>
                  </div>
                </div>
              </div>
