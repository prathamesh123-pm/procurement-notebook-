...
export default function WordEditorPage() {
  ...
  const RibbonGroup = ({ label, children }: { label: string, children: React.ReactNode }) => (
    <div className="flex flex-col items-center border-r border-slate-200 px-1 last:border-0 h-full shrink-0">
      ...
      <span className="text-[6px] font-black uppercase text-slate-400 mt-0.5 tracking-tighter">{label}</span>
    </div>
  )
  ...
  const BigToolBtn = ({ icon: Icon, onClick, label }: any) => (
    ...
    >
      ...
      <span className="text-[6px] font-black uppercase mt-0.5 leading-none">{label}</span>
    </Button>
  )
  ...
  const tabs = [
    ...
    { id: "dev", label: "Dev" },
    { id: "help", label: "Help" },
    { id: "addins", label: "Addins" }
  ]

  return (
    <div className="flex flex-col h-[calc(100vh-100px)] max-w-5xl mx-auto w-full bg-slate-50 overflow-hidden rounded-2xl border shadow-xl animate-in fade-in duration-500">
      <div className="h-6 bg-primary text-white flex items-center justify-between px-2 shrink-0">
        <div className="flex items-center gap-1 min-w-0">
          <div className="p-0.5 bg-white rounded shadow-sm shrink-0"><FileText className="h-2 w-2 text-primary" /></div>
          <Input 
            value={docTitle} 
            onChange={(e) => setDocTitle(e.target.value)}
            className="h-4 bg-transparent border-none text-[9px] font-black uppercase text-white placeholder:text-white/50 w-24 focus-visible:ring-0 p-0 truncate"
          />
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <div className="hidden sm:flex items-center gap-1 text-[7px] font-black opacity-70 uppercase"><Shield className="h-1.5 w-1.5" /> Protected</div>
          <Button variant="ghost" size="icon" className="h-4 w-4 text-white hover:bg-white/10" onClick={() => window.location.reload()}><RefreshCw className="h-2 w-2" /></Button>
        </div>
      </div>
      ...
      <Tabs defaultValue="home" className="w-full shrink-0 bg-white border-b shadow-sm">
          ...
          <TabsList className="h-6 bg-transparent justify-start px-1 gap-0 overflow-visible">
            {tabs.map((tab) => (
              <TabsTrigger 
                key={tab.id} 
                value={tab.id} 
                className="text-[7px] font-black uppercase h-full px-2 data-[state=active]:bg-white data-[state=active]:text-primary rounded-t-md rounded-b-none transition-all border-x border-transparent data-[state=active]:border-slate-200"
              >
                {tab.label}
              </TabsTrigger>
            ))}
        ...
        <div className="h-12 bg-white p-0.5 overflow-hidden">
          ...
          <TabsContent value="home" className="m-0 h-full">
            <div className="flex items-center gap-0.5 h-full px-1">
              ...
              <RibbonGroup label="FONT">
                <select className="h-5 text-[8px] border rounded bg-slate-50 px-0.5 outline-none font-bold" onChange={(e) => execCommand('fontName', e.target.value)}>
                  <option value="Inter">Inter</option>
                  <option value="Arial">Arial</option>
                </select>
                <ToolBtn icon={Bold} label="B" onClick={() => execCommand('bold')} />
                <ToolBtn icon={Italic} label="I" onClick={() => execCommand('italic')} />
                <ToolBtn icon={Underline} label="U" onClick={() => execCommand('underline')} />
                <ToolBtn icon={Highlighter} label="H" onClick={() => execCommand('backColor', 'yellow')} />
              </RibbonGroup>
              <RibbonGroup label="PARA">
                <ToolBtn icon={AlignLeft} label="L" onClick={() => execCommand('justifyLeft')} />
                <ToolBtn icon={AlignCenter} label="C" onClick={() => execCommand('justifyCenter')} />
                <ToolBtn icon={AlignRight} label="R" onClick={() => execCommand('justifyRight')} />
          ...
          <TabsContent value="insert" className="m-0 h-full">
            <div className="flex items-center gap-0.5 h-full px-1">
              ...
              <RibbonGroup label="FORM">
                <ToolBtn icon={Box} label="Box" />
                <ToolBtn icon={FileSignature} label="Sign" onClick={() => execCommand('insertHTML', '<div style="margin-top:20px;">___________________<br/>Sign</div>')} />
              </RibbonGroup>
            </div>
          </TabsContent>
          ...
          <TabsContent value="view" className="m-0 h-full">
            <div className="flex items-center gap-1 h-full px-2">
              <RibbonGroup label="ZOOM">
                ...
                <span className="text-[8px] font-black w-6 text-center">{zoom}%</span>
      ...
      <ScrollArea className="flex-1 bg-slate-200 shadow-inner p-1 sm:p-2">
        <div 
          className="bg-white mx-auto shadow-xl min-h-[800px] p-4 sm:p-8 focus:outline-none transition-all duration-300"
          style={{ 
        ...
        >
          <div 
            ...
            className="w-full h-full min-h-[700px] text-xs prose prose-xs max-w-none focus:outline-none font-body"
            onInput={() => {}} 
          >
            <h2 className="text-center" style={{margin: '0 0 5px 0'}}>संकलन नोंदवही (Official Report)</h2>
            <p className="text-center text-muted-foreground text-[8px] uppercase font-black tracking-widest" style={{margin: '0 0 10px 0'}}>दैनिक अहवाल व मार्गदर्शिका</p>
      ...
      <div className="h-4 bg-slate-100 border-t flex items-center justify-between px-2 text-[7px] font-black text-slate-500 uppercase shrink-0">
        <div className="flex items-center gap-2">
          <span>PAGE 1 OF 1</span>
          <span className="hidden sm:inline">WORDS: {editorRef.current?.innerText.trim().split(/\s+/).length || 0}</span>
          <span className="flex items-center gap-1"><Check className="h-1.5 w-1.5 text-green-600" /> AUTOSAVE ON</span>
        </div>
